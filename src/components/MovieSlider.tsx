...
const MovieSlider = () => {
  ...
  return (
    <div className=" relative max-w-[1440px] mt-20 py-4 mx-auto md:min-h-[785px] flex flex-col items-center justify-center md:mt-[192px]">
      <img
        className="h-full z-0 w-full object-cover object-center absolute"
        src={`https://image.tmdb.org/t/p/w500${currentMovie?.poster_path}`}
        alt=""
      />
     
      <div className="absolute top-0 left-0 z-10 h-full w-full bg-primaryBlack bg-opacity-[0.8]">

      </div>
      <div className="space-y-2  py-7 z-20 px-5 lg:hidden">
      ...
      <div className=" flex flex-col-reverse lg:flex-row lg:gap-7 md:pl-10 z-20">
        <div className="space-y-[74px] max-w-[487px] w-full px-[24px] md:px-0 ">
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <h1
                className={`text-[22px] font-groteska-bold  text-primaryWhite  md:${
                  currentMovie?.title && currentMovie?.title.length > 30
                    ? "text-3xl"
                    : "text-[44px]"
                }`}
              >
        ...
        <div className="w-full z-20 overflow-x-hidden flex gap-x-4 px-5 md:px-0">
          {trendingMovies?.map((movie, index) => (
            <AnimatePresence mode="wait">
              <motion.img
                key={movie.id}
                className={`w-[300px] h-[400px] object-cover object-center rounded-xl transition-all ease-in-out duration-200 md:w-[371px] md:h-[548px] ${
                ...
                transition={{ duration: 0.5 }} // Transition between states
                style={{
                  display:
